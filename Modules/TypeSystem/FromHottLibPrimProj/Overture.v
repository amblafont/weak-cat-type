...
(* Primitive projections are necessary to make functional relation faster
(See FullBrunerieIsContrForallSyntax for slow example)

 *)
Set Primitive Projections.

Record sigT (A : Type) (P : A -> Type) := existT { pr1 : A ; pr2 :  P pr1 }.
...
Record prod (A : Type) (B : Type) := pair { fst : A ; snd :  B}.
...
Definition iff (A B : Type) := ((A -> B) * (B -> A))%type.
Infix "<->" := iff.
...
(* Check ((0 ; eq_refl 0) : sigT (fun x => x = x)). *)
...
Global Instance reflexive_paths {A} : Reflexive (@eq A) | 0 := @idpath A.
...
Ltac path_induction :=
  intros; repeat progress (
    match goal with
      | [ p : ?x = ?y  |- _ ] => not constr_eq x y; destruct p
    end
  ).
