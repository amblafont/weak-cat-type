...
: u = v.
  destruct u as [u1 u2] , v as [v1 v2].
  cbn in pq.
  destruct pq.2.
  destruct pq.1.
  exact 1.
...
Instance isequiv_path_sigma_contra `{P : A -> Type} {u v : sigT P}
  ...
  (* destruct u as [u1 u2]. *)
     by intro r; destruct r; destruct u as [u1 u2]; reflexivity.
  ...
  unshelve refine (
   @BuildEquiv
       ...
       (@BuildIsEquiv
          ...
          (fun apq => match apq with ((x ; y) ; z) => (x; (y; z)) end)
          (fun apq => _)
          (fun apq => match apq with (x ; (y ; z) ) => 1 end)
          (fun apq => _)
       )
       )
  .
  - destruct apq as [[a p] q]; exact 1.
  - destruct apq as [a [p q]]; exact 1.
Defined.
...
: {a : A & {b : B & Q (a,b)}} <~> sigT Q.
  ...
  @BuildEquiv
       ...
       (@BuildIsEquiv
          {a : A & {b : B & Q (a,b)}} (sigT Q)
          (fun apq => ((apq.1, apq.2.1); apq.2.2))
          (fun apq => match apq with
                     ((x , y) ; z) => (x ; (y ; z))
                   end)
                     (* (fst apq.1; (snd apq.1; apq.2))) *)
          (fun abq => _)
          (fun apq => match apq with (x ; (y ; z) ) => 1 end)
          (fun abq => _))
  ).
  - destruct abq as [[a b] q]; exact 1.
  - destruct abq as [a [b q]]; exact 1.
Defined.
...
Proof.
  ...
  refine (ap (fun p => match p in (_ = v2) return (_ = (u1; v2)) with 1 => 1 end)
             (contr (idpath u2))).
Defined.
